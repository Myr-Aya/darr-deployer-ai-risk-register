...
Before any automated classification, all 1,835 entries were read in full, grouped by MIT sub-domain, so that the canonical taxonomy was grounded in the actual content rather than imposed from outside. This produced **61 canonical deployer risks** organized along the seven MIT domains plus an organizational-governance cluster. The granularity sits between the MIT sub-domain level (around 24 themes, too coarse for an organization's working register) and the raw entry level (1,219 distinct free-text labels, too fine for an organization's working register). Examples of the consolidation logic:
...
A human review of approximately 9.3% of the classifications was carried out: 171 entries checked, including 20 of the 252 entries excluded by Filters 1 and 2. The main finding was a 20% disagreement rate, mostly on entries the classifier's conservative approach kept but that should have been excluded under Filter 1 or 2. The review protocol and its results are planned for release as part of the v2 methodology.
...
Part One drew on the MIT repository, which catalogues AI harms and failure modes. The management-system and life-cycle process risks that a deploying organization is governed and audited against are a complementary dimension. Part Two adds that dimension by reading the 61 canonical risks against two standards: ISO/IEC 23894:2023 (AI risk management guidance) and ISO/IEC 42001:2023 (the auditable AI management system standard). The aim was twofold: identify risks not covered by the 61 (gaps), and attach structural ISO metadata to those that are covered (enrichment). The 42001 Annex A controls were treated as the priority input because they are normative and are what deployers are certified against.
